...
@section('content')
    <div class="card ">
        ...
        <div class="card-body ">
            ...
            {!! Form::open(['route' => 'beneficiary.report.result.search_date_between' ,'action'=>'get' ,'id'=>'formSearch','class'=>'']) !!}
            ...
            <div class="col-md-6">
                <div class="row">
                    <label for="date_start" class="col-form-label col-md-4">Start Date </label>
                        <div class="form-group  col-md-8">
                            <input type="text" value="" class="form-control  datetimepicker " name="date_start"
                                   id="date_start">
                        </div>
                </div>
            </div>
            ...
            <div class="col-md-6">
                <div class="row">
                    <label for="date_end" class="col-form-label  col-md-4">End Date </label>
                    <div class="form-group   col-md-8">
                        <input type="text" value="" class="form-control  datetimepicker " name="date_end" id="date_end">
                    </div>
            ...
            <div class="col-md-12">
                ...
                <a href="{{route('reports.prepare',12)}}" class="btn btn-danger btn-sm "
                ...
                <a href="{{route('beneficiary.report.result.btnReportPDF_date_between')}}"
                ...
                <a href="{{route('beneficiary.report.result.reportExportExcel_date_between')}}"
...
@section('script')
    <script>
        ...
        $(document).ready(function () {
            ...
            active_nev_link('beneficiary_report_result_value_date_between')
        });


            $("#date_start").on("dp.change", function (e) {
             var date_start = $('#date_start').val();
             $('#date_end').data("DateTimePicker").minDate(date_start);
         });
        ...
        $(document).on('submit', '#formSearch', function (e) {
            e.preventDefault();
            data = $(this).serialize();
            var url = '{{route("beneficiary.report.result.search_date_between")}}';
            $.ajax({
                url: url,
                data: data,
                type: 'get',
        ...
        $(document).on('click', '#btnReportExcel', function (e) {
            ...
            var url = '{{route("beneficiary.report.result.reportExportExcel_date_between")}}' + '?' + data;
        ...
        $(document).on('click', '#btnReportPdf', function (e) {
            e.preventDefault();
            data = $('#formSearch').serialize();
            var url = '{{route("beneficiary.report.result.btnReportPDF_date_between")}}' + '?' + data;
            window.location.href = url;
        ...
        $(document).on('click', '#btnValue', function (e) {
            e.preventDefault();
            data = $('#formSearch').serialize();
            var url = '{{route("beneficiary.report.result.reportValueResult_date_between")}}'+'?'+data;
            window.location.href = url;
        });


        var reports_getData = "{{route('reports.getData',12)}}";
