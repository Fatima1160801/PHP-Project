...
<?php $__env->startSection('script'); ?>
    <script>
        ...
        $('#formDashboardProjectsFilter').submit(function(e){
            //$('#project-plus-page').data('filteron',1); //setter
            $('#project-plus-page').removeAttr("data-filteron" );
            $('#project-plus-page').attr("data-filteron",1);
            $page=$('#project_paginate_val').val();
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('.loader').hide();
                    if(data.status==true) {
                        $("#project-plus-page").css("display","block");
                        $("#drawProjectMore").html(data.html);
                    }else{
                        $("#project-plus-page").css("display","none");
                        $("#drawProjectMore").html("<h5>No Projects to display</h5>");
                    }
        ...
        $('#formDashboardActivitiesFilter').submit(function(e){
            e.preventDefault();
            $page=$('#activity_paginate_val').val();
            var form = $(this).serialize();
            var url = $(this).attr('action');
            $.ajax({
                url: url+"?page="+$page,
                ...
                success: function (data) {
                    ...
                    $('.loader').hide();
                    if(data.status==true){
                        $("#activity-plus-page").css("display","block");
                        $("#drawActivityMore").html(data.html);
                        $("#activity_paginate_val").val($page);
                        //$("#drawActivityMore").html(data.html);
                    }else{
                        $("#activity_paginate_val").val($page);
                        $("#activity-plus-page").css("display","none");
                        $("#activity-min-page").css("display","block");
                        $("#drawActivityMore").html("<h5>No Activities to display</h5>");
                    }
    ...
    <script src="<?php echo e(asset('assets/js/plugins/fullcalendar.min.js')); ?>"></script>
     ...
     <script>
         ...
         $(document).on('click', '#clear-project-filter', function (e) {
             $("#project_paginate_val").val(0);
              getPaginateProject(1);
             $("#project-plus-page").css("display","block");
             $("#formDashboardProjectsFilter")[0].reset();
             $('#formDashboardProjectsFilter .selectpicker').selectpicker("refresh");
             $('#project-plus-page').removeAttr("data-filteron" );
             $('#project-plus-page').attr("data-filteron",0);
            // $("#formDashboardProjectsFilter").selectpicker("refresh")
         });

         $(document).on('click', '#clear-activity-filter', function (e) {
             $("#activity_paginate_val").val(0);
             getPaginateActivity(1);
             $("#activity-plus-page").css("display","block");
             $("#formDashboardActivitiesFilter")[0].reset();
             $('#formDashboardActivitiesFilter .selectpicker').selectpicker("refresh");
             $('#activity-plus-page').removeAttr("data-filteron" );
             $('#activity-plus-page').attr("data-filteron",0);
             // $("#formDashboardProjectsFilter").selectpicker("refresh")
         });
