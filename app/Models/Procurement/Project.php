            ...
            if ($userPermData != null) {
                if ($userPermData->perm_type == 1) {
                    ...
                    $projects_ids = UserDataPermissionModule::where('user_id', $user_id)
                        ->where('module_id', 1)->pluck('primary_id')
                        ->unique()
                        ->toArray();

                    // created by
                    $projects_created_by  =  Project::where('created_by',$user_id)->pluck('id')->toArray();

                    $projects_ids = array_merge($projects_created_by,$projects_ids);
