...
indicatorsWizard = {
    initMaterialWizard: function () {
        ...
        function refreshAnimationGoal($wizard, index) {
            ...
            $('#wizardIndicators').find('.nav-project li').css('float','right');

            step_width = move_distance;
            // move_distance = move_distance * index_temp;
            move_distance =   $('#wizardIndicators').width() - ((index_temp+1) * (move_distance));
    ...
    initMaterialWizard: function () {
        ...
        function refreshAnimationWizardProject($wizard, index) {
         ...
         //   $total_wizardIndicators = $('#wizardIndicators').find('.nav-project li').length
            ...
            mobile_device = $(document).width() < 600 && $total_wizardProject > 3;
            ...
            $('#wizardProject').find('.nav-project li').css('float','right');

            step_width = move_distance;
           // move_distance = move_distance * index_temp;
            move_distance =   $('#wizardProject').width() - ((index_temp+1) * (move_distance));
            $current = index + 1;
...
donorsWizard = {
    initMaterialWizard: function () {
        ...
        function refreshAnimationGoal($wizard, index) {
            ...
            $('#wizardDonors').find('.nav-pills li').css('float','right');

            step_width = move_distance;
            // move_distance = move_distance * index_temp;
            move_distance =   $('#wizardDonors').width() - ((index_temp+1) * (move_distance));
...
wizardActivity = {
    initMaterialWizard: function () {
        ...
        function refreshAnimationGoal($wizard, index) {
            ...
            $('#wizardActivity').find('.nav-project li').css('float','right');

            step_width = move_distance;
            // move_distance = move_distance * index_temp;
            move_distance =   $('#wizardActivity').width() - ((index_temp+1) * (move_distance));
...
console.log(2555);
        ...
        $('[data-toggle="wizard-radio"]').click(function () {
            wizard = $(this).closest('#wizardStrategic');
            wizard.find('[data-toggle="wizard-radio"]').removeClass('active');
            $(this).addClass('active');
            $(wizard).find('[type="radio"]').removeAttr('checked');
            $(this).find('[type="radio"]').attr('checked', 'true');
        ...
        $(window).resize(function () {
            $('#wizardStrategic').each(function () {
                $wizard = $(this);

                index = $('#wizardStrategic').bootstrapWizard('currentIndex');
        ...
        function refreshAnimationGoal($wizard, index) {
            ...
            $total = $('#wizardStrategic').find('.nav-pills li').length;
            $li_width = 100 / $total;

            total_steps = $('#wizardStrategic').find('.nav-pills li').length;

            move_distance = $('#wizardStrategic').width() / total_steps;
            ...
            if (mobile_device) {
                move_distance = $('#wizardStrategic').width() / 2;
                index_temp = index % 2;
                $li_width = 50;
            }

            $('#wizardStrategic').find('.nav-pills li').css('width', $li_width + '%');
            $('#wizardStrategic').find('.nav-pills li').css('float','right');

            step_width = move_distance;
            // move_distance = move_distance * index_temp;
            move_distance =   $('#wizardStrategic').width() - ((index_temp+1) * (move_distance));
            ...
            $('#wizardStrategic').find('.moving-tab-goals').css('width', step_width);
            $('.moving-tab-goals').css({
                'transform': 'translate3d(' + move_distance + 'px, ' + vertical_level + 'px, 0)',
                'transition': 'all 0.5s cubic-bezier(0.29, 1.42, 0.79, 1)'
